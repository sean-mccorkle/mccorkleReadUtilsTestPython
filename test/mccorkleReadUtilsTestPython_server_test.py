...
class mccorkleReadUtilsTestPythonTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'mccorkleReadUtilsTestPython',
        ...
        for nameval in config.items('mccorkleReadUtilsTestPython'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = mccorkleReadUtilsTestPython(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_mccorkleReadUtilsTestPython_" + str(suffix)
